...
export default function Home({ allSpots = [], onNavigate }) {
  const [searchQuery, setSearchQuery] = useState('');

  // Enhanced mock data with realistic ratings
  const enhancedSpots = useMemo(() => {
    return (allSpots || []).map(spot => ({
      ...spot,
      rating: spot.rating || (Math.random() * 2 + 3.5), // Random rating 3.5-5.5
      reviewCount: spot.reviewCount || Math.floor(Math.random() * 200) + 20,
      distance: spot.distance || (Math.random() * 50 + 1).toFixed(1) + ' km'
    }));
  }, [allSpots]);
  ...
  const filteredSpots = useMemo(() => {
    ...
    const validSpots = enhancedSpots.filter(spot => 
      spot && typeof spot === 'object' && spot.spot_id
    );
